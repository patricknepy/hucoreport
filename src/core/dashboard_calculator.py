...
class DashboardCalculator:
    ...
    def get_warnings_evolution(self) -> Dict[str, Any]:
        ...
        Returns:
            Dict avec 'weeks', 'warning_client', 'warning_internal', 'total_warnings'
        """
    ...
    def get_warnings_by_month(self) -> Dict[str, Any]:
        ...
        # Organiser les semaines par mois
        # On utilise l'année courante pour la conversion semaine -> mois
        current_year = datetime.datetime.now().year

        weeks_by_month = {}
        for week in sorted(available_weeks):
            # Calculer la date du lundi de cette semaine
            try:
                # ISO week: lundi de la semaine N
                monday = datetime.datetime.strptime(f'{current_year}-W{week:02d}-1', '%G-W%V-%u')
